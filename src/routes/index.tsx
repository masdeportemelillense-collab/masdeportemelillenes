...
function Home() {
  ...
  return (
    <div className="space-y-10">
      <section className="relative overflow-hidden rounded-2xl bg-surface px-5 py-8 sm:px-8 sm:py-10">
        ...
        <div className="relative max-w-2xl">
          <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-accent">
            Temporada 2026-27 · Ciudad Autónoma
          </p>
          <h1 className="mt-3 font-display text-5xl leading-[0.9] sm:text-6xl">
            El deporte de Melilla, en directo
          </h1>
          <p className="mt-4 max-w-lg text-sm text-muted sm:text-base">
            Marcadores oficiales en tiempo real de UD Melilla y Melilla Baloncesto, más calendario y
            clasificación del resto de equipos de la ciudad.
          </p>
        </div>
      </section>
      ...
      {live.length > 0 && (
        <section>
          <SectionHead title="En directo ahora" hint={`${live.length} en juego`} />
      ...
      {todayRest.length > 0 && (
        <section>
          <SectionHead title="Hoy" hint="Hora de Madrid" />
          <div className="grid gap-3 md:grid-cols-2">
            {todayRest.map((m) => (
              <MatchCard key={m.id} match={m} />
            ))}
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {SPORTS.map((s) => {
            ...
            return (
              ...
              >
                <SportMark sport={s.id} className="size-5 text-accent" />
      ...
      {restUpcoming.length > 0 && (
        <section>
          <SectionHead title="Próximos partidos" />
          <div className="grid gap-3 md:grid-cols-2">
            {restUpcoming.map((m) => (
              <MatchCard key={m.id} match={m} />
            ))}
      ...
      <section>
        <SectionHead title="Últimos resultados" />
        <div className="grid gap-3 md:grid-cols-2">
      ...
      <section>
        <SectionHead title="Todos los equipos" />
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {teams.map((t) => (
            <TeamChip key={t.id} id={t.id} />
          ))}
        </div>
      </section>
    </div>
...
function SectionHead({
  ...
  return (
    <div className="mb-4 flex items-end justify-between gap-3">
      ...
      {hint && <p className="text-xs uppercase tracking-wider text-muted">{hint}</p>}
    </div>
  );
}
...
function TeamChip({ id }: { id: string }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-surface py-2 pr-2 pl-3 shadow-[var(--shadow-border)]">
      ...
      >
        ...
        <span className="min-w-0">
          ...
          <span className="flex items-center gap-1.5 text-xs text-muted">
            <SportMark sport={team.sport} className="size-3" />
            {sportLabel[team.sport]} · {team.category}
          </span>
        </span>
      ...
      >
        <Star className={fav ? "size-4 fill-accent text-accent" : "size-4"} />
      </button>
    </div>
  );
}
